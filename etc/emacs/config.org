...
** Disable Electric Pairing of <>

Basically when we being src blocks in =org-mode=, it appears that due the electric pair feature, we end up adding an extra =>= to the block which prevents us from immediately editing it. Extremely useful specially when we are using structure templates
