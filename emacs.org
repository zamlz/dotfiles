...
#+LAST_MODIFIED: [2021-07-22 Thu 07:23:14]
#+STARTUP: content
...
* Summon the Daemon
:PROPERTIES:
:header-args:shelly: :tangle ~/.config/xinitrc.d/emacs-daemon-refresh.sh :mkdirp yes :shebang #!/bin/sh :comments both
:END:
...
#+begin_src shell
if [ -z "$(pgrep -f 'emacs --daemon=xorg-emacs-daemon --with-x-toolkit=lucid')"]; then
    emacs --daemon=xorg-emacs-daemon --with-x-toolkit=lucid
fi
#+end_src
